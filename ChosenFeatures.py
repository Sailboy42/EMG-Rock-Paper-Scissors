...
def mean_frequency(data):
    ...
    p = np.abs(sp)**2
    mean_freq = np.sum(p * freq.reshape((1, -1, 1)), axis=1) / np.sum(p, axis=1)
    return mean_freq.T
...
def autoregressive_coefficients(data):
    ...
    ar_coeffs = np.zeros((num_trials, num_channels))

    for i in range(num_channels):
        for j in range(num_trials):
            model = AutoReg(data[i, :, j], lags=1)
            model_fitted = model.fit()
            ar_coeffs[j, i] = model_fitted.params[1]
...
def extract_features(data):
    ...
    combined_features = np.hstack((var, mav, rms, zc, wl, mf, ar))
